...
function renderScene() {
	...
	// 2. Рисуем белую обводку выбранного объекта ПОСЛЕ всех спрайтов → она будет сверху
	if (selectedObject && selectedObject.parent) {
		...
		if (img && img.complete) {
			...
			ctx.lineWidth = 1 / viewPPU; // обводка в 2 пикселя экрана
			ctx.setLineDash([]);
...
function renderSpritesToBase64(ignoreNameList = [], alphaThreshold = 1) {
	...
	const sortedObjects = [...sceneObjects].sort((a, b) => a.sortingOrder - b.sortingOrder);
	sortedObjects.forEach(o => {
		if (!o?.texture || o.enabled === false || o.isActive === false || o.spritePreview || o.parent && sortedObjects.find(obj => obj.name === o.parent)?.isActive === false) return;
	...
	return out.toDataURL('image/png');  // Превращаем PNG в BASE64  // "data:image/png;base64,XXXX"
}
...
canvas.addEventListener('mousedown', (e) => {
	...
	for (const o of sortedForHit) {
		if (o.canChangePivot) {
			...
			const dist = Math.sqrt((wx - pivot) ** 2 + (wy - pivot) ** 2);
			if (dist < 10 / viewPPU) {
				pivotHitObject = o;
				break;
			}
