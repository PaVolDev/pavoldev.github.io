...
function inputMinMax(input) {
	if (input.value != "") {
		...
		if (input.hasAttribute('max') && parseFloat(input.value) > parseFloat(input.max)) {
...
function updateValueByPath(newValue, path) {
	let index = -1;
	// 1. Поиск индекса элемента в массиве
	if (typeof path === "string") {
		index = editedParams.findIndex(p => p.fieldPath === path || p.startFieldPath === path);
	}
	if (index === -1) {
		const parsedIndex = parseInt(path);
		if (!isNaN(parsedIndex)) index = parsedIndex;
	}
	// 2. Если элемент найден (по строке или индексу цифрой)
	if (index !== -1 && editedParams[index]) {
		const param = editedParams[index];
		// Обработка числовых типов (вынесено в один блок)
		if (param.type === "int" || param.type === "float") {
			newValue = parseFloat(newValue);
			newValue = ("min" in param) ? Math.max(param.min, newValue) : newValue;
			newValue = ("max" in param) ? Math.min(param.max, newValue) : newValue;
		}
		param.value = newValue;
		return param;
	}
	// 3. Обработка вложенных путей (объект.свойство)
	const pathList = (typeof path === "string") ? path.split('.') : path;
	if (Array.isArray(pathList) && pathList.length > 0) {
		index = editedParams.findIndex(p => p.fieldPath === pathList[0] || p.startFieldPath === pathList[0]);
		if (index !== -1) {
			pathList.shift();
	...
	// 4. Лог ошибки, если ничего не подошло
	const logPath = Array.isArray(path) ? path.join('/') : path;
	console.warn(`updateValueByPath: параметр не найден для пути: ${logPath}; Тип: <${typeof path}>, parseInt = ${parseInt(path)}`);
	return null;
}
...
function updateValueByPath(newValue, path) {
	...
	if (typeof path === "string" && (index = editedParams.findIndex(p => p.fieldPath === path || p.startFieldPath === path)) != -1) {
		...
		if ((editedParams[index].type == "int" || editedParams[index].type == "float")){
			newValue = parseFloat(newValue);
			newValue = ("min" in editedParams[index]) ? Math.max(editedParams[index].min, newValue) : newValue;
			newValue = ("max" in editedParams[index]) ? Math.min(editedParams[index].max, newValue) : newValue;
		}
		return editedParams[index];
	}
	index = parseInt(path);
